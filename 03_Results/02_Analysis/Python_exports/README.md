# 03_Results/02_Analysis/Python_exports — intermediate R-to-Python GSEA bridge files

## Overview

This directory contains two CSV exports of GSEA results produced by the R pipeline for consumption by the Python master-table builder. These are **intermediate artifacts**; downstream Python visualization scripts should read `../master_gsea_table.csv` rather than these files directly. They are also used by the 5a AveExpr diagnostics scripts to build the expression-filter audit tables in `Supplementary/`.

**Generating script:** `02_Analysis/1.4.export_gsea_for_python.R`
